...
$removeurl = $plugin_url . 'region-remove.php';
$editurl = $plugin_url . 'region-update.php';
...
echo '<div id="regiontable" class="wrap">';
...
echo '<div id="regionedit" style="display:none" >';
    echo '<h2>Edit Region</h2>';
    echo "<form id='myFormEdit' action='$editurl' method='post'>";
        echo '<label>ID:</label><br>';
        echo '<input id="editregionid" type="text" name="editregionid" readonly><br><br>';
        echo '<label>Name:</label><br>';
        echo '<input id="editregionname" type="text" name="editregionname"><br>';
        echo "<button type='submit' style='cursor:pointer;padding:10px;background-color:#6495ed;color:white;width:100px;height:45px;text-align:center;margin-top:20px;border:none;'>Update</button>";
    echo '</form>';
echo '</div>';
...
class Region_List_Table extends WP_List_Table
{
    ...
    {
        $actions = array(
            'edit' => sprintf('<div style="display: inline-block;color:blue;cursor: pointer;" onclick="regioneditbox(%s,\'%s\')">Edit</div>', $item['id'],  $item['name']),
